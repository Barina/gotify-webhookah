...
// No server-side config needed anymore — domain override lives in the UI.
type Config struct{}
...
func (p *Plugin) serveBuilder(c *gin.Context) {
	...
	serverBase := fmt.Sprintf("%s://%s", scheme, c.Request.Host)
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .toggle-row { display: flex; align-items: center; gap: 0.6rem; margin-top: 0; margin-bottom: 1rem; }
  ...
  <div class="card">
    ...
    <div class="cmd-block">
      <div class="cmd-label">
        <span>Command</span>
        <div class="btn-row">
          <button class="action-btn" id="copyCmd" onclick="copyCmd()">Copy</button>
          <button class="action-btn" id="testCmd" onclick="testCmd()">Test</button>
        </div>
      </div>
      <div class="cmd-text empty" id="curlCmd">Select an app and enter a message to generate</div>
    </div>
    <div class="note">
      <strong>Note:</strong> This is a <code>curl</code> command for scripts, CI/CD, or terminals.
      ...
      When markdown is enabled, the command sends a <strong>JSON body</strong> with the correct extras header.
    </div>
  </div>
</div>
...
function restoreFields(s) {
  if (!s) return;
  if (s.message) document.getElementById('message').value = s.message;
  if (s.title) document.getElementById('title').value = s.title;
  if (s.priority !== undefined && s.priority !== '') document.getElementById('priority').value = s.priority;
  if (s.domainOverride) document.getElementById('domainOverride').value = s.domainOverride;
  if (s.markdown) document.getElementById('markdownToggle').checked = s.markdown;
}

function restoreAppSelection(s) {
  if (!s || !s.appToken) return;
  const sel = document.getElementById('app');
  for (let i = 0; i < sel.options.length; i++) {
    if (sel.options[i].value === s.appToken) { sel.selectedIndex = i; break; }
  }
}
...
function getBase() {
  ...
  if (override) {
    const scheme = serverBase.startsWith('https') ? 'https' : 'http';
...
function buildCurlCmd(p) {
  if (!p) return null;
  if (p.isJSON) {
    const escaped = p.body.replace(/'/g, "'\\''");
    return "curl -X POST '" + p.url + "' \\\n  -H 'Content-Type: application/json' \\\n  -d '" + escaped + "'";
  }
  return "curl -X POST '" + p.url + "'";
}
...
function updateCommands() {
  ...
  if (!currentPayload) {
    el.textContent = 'Select an app and enter a message to generate';
    el.className = 'cmd-text empty';
    return;
  }
  el.textContent = buildCurlCmd(currentPayload);
  el.className = 'cmd-text';
}
...
async function testCmd() {
  ...
  const btn = document.getElementById('testCmd');
  btn.textContent = 'Sending...';
  btn.className = 'action-btn testing';
  try {
    const opts = { method: 'POST' };
    if (currentPayload.isJSON) {
      opts.headers = { 'Content-Type': 'application/json' };
      opts.body = currentPayload.body;
    }
    const resp = await fetch(currentPayload.url, opts);
    btn.textContent = resp.ok ? 'Sent!' : 'Failed ' + resp.status;
    btn.className = 'action-btn ' + (resp.ok ? 'sent' : 'failed');
  } catch(e) {
...
</html>`, v, serverBase, appsEndpoint)
